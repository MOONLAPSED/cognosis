...
"""
[[Formal Theory]]
The operators `=` `⧧` `if then else fi` apply to every type of expression (but the first operand of if then else f must be binary), with the laws. The `Formal Theory` this object and all children with [[double-brackets]] always have a static file-system representation, totally seperate from the python or any other runtime, which is `.md` files within the current, parent, or grandparent directies which work with Obsidian and represent artifacts of a working obsidian 'knowledge base'

[[Reflexivity]]
x=x

[[Case Base]]
if ⊤ then x else y fi = x
if ⊥ then x else y fi = y
...
[[Transparency]]
x=y ⇒ f x = f y
if a then x else y fi = if ¬a then y else x fi
...
[[One Case]]
if a then ⊤ else b fi = a∨b

[[Case Absorption]]
if a then b else c fi = if a then a∧b else c fi
if a then b else c fi = if a then a⇒b else c fi
