...
class FormalTheory(Generic[T], Atom):
    ...
    statement: str
    prediction: Callable[..., bool] = field(default_factory=lambda: lambda color, location: color == "white")
...
theory = FormalTheory(
    top_atom=top_atom,
    bottom_atom=bottom_atom,
    statement="All observed swans are white",
    prediction=lambda color, location: color == "white"
)

# Test the theory
result = theory.test("white", "Europe")
print(f"Test result: {result}")  # Should print the top_atom (True)

result = theory.test("black", "Europe")
...
print(f"Refined test result: {result}")  # Should return the top_atom (True) depending on the implementation
